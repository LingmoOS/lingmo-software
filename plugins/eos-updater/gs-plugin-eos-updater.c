 ...
 * context from lingmo-software’s main thread. This means all the signal
 * callbacks from the proxy will be executed in the main thread, and *must not
 * block*.
 *
 * Asynchronous plugin vfuncs (such as
 * gs_plugin_eos_updater_refresh_metadata_async()) are run in lingmo-software’s
              ...
              gpointer      user_data)
{
	...
	gs_app_set_metadata (app, "LingmoSoftware::UpgradeBanner-css", css);
