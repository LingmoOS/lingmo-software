...
gs_plugin_packagekit_init (GsPluginPackagekit *self)
{
	...
	self->settings_proxy = g_settings_new ("org.lingmo.system.proxy");
	g_signal_connect (self->settings_proxy, "changed",
			  G_CALLBACK (gs_plugin_packagekit_proxy_changed_cb), self);

	self->settings_http = g_settings_new ("org.lingmo.system.proxy.http");
	self->settings_https = g_settings_new ("org.lingmo.system.proxy.https");
	self->settings_ftp = g_settings_new ("org.lingmo.system.proxy.ftp");
	self->settings_socks = g_settings_new ("org.lingmo.system.proxy.socks");
		       ...
		       GError **error)
{
	...
	for (i = 0; i < array->len; i++) {
		...
		if (app == NULL) {
			...
			gs_app_set_metadata (app, "LingmoSoftware::SortKey", "300");
	...
	for (i = 0; i < gs_app_list_length (list); i++) {
		...
		if (!ret) {
			...
			if (gs_app_get_state (app) == GS_APP_STATE_INSTALLED) {
				...
				gs_plugin_packagekit_set_packaging_format (plugin, app);
				gs_app_set_metadata (app_dummy, "LingmoSoftware::Creator",
						     gs_plugin_get_name (plugin));
		       ...
		       GError **error)
{
	...
	gs_app_set_metadata (app, "LingmoSoftware::packagename-value", packagename);
