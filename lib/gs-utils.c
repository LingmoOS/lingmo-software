 ...
 * -- lingmo-software will not ever clean the cache for the plugin.
...
gs_utils_get_user_hash (GError **error)
{
	...
	salted = g_strdup_printf ("lingmo-software[%s:%s]",
 ...
 * @id: A PolicyKit ID, e.g. "org.lingmo.Desktop"
 ...
 *
 * Returns: the user-agent, e.g. "lingmo-software/3.22.1"
 */
const gchar *
gs_user_agent (void)
{
 ...
 * @origin: Origin string, e.g. 'os' or 'lingmo-apps-nightly'
...
#define METADATA_ETAG_ATTRIBUTE "xattr::lingmo-software::etag"
 ...
 * be used by lingmo-software. See `doc/vendor-customisation.md`.
 *
...
gs_utils_get_upgrade_background (const gchar *version)
{
	...
	if (version != NULL) {
		filename = g_strdup_printf (DATADIR "/lingmo-software/backgrounds/%s-%s.png", os_id, version);
		if (g_file_test (filename, G_FILE_TEST_EXISTS))
			return g_steal_pointer (&filename);
		g_clear_pointer (&filename, g_free);
	}

	filename = g_strdup_printf (DATADIR "/lingmo-software/backgrounds/%s.png", os_id);
	if (g_file_test (filename, G_FILE_TEST_EXISTS))
		return g_steal_pointer (&filename);
	g_clear_pointer (&filename, g_free);
