 ...
 * component. For Lingmo Software specific app-like entries, which don't correspond to desktop
 ...
 * a #GsApp in different parts of lingmo-software can be observed by connecting to
 ...
 *
 * Returns: The unique ID, e.g. `system/flatpak/flathub/org.lingmo.Notes/stable`, or %NULL
 *
 * Since: 3.22
 **/
const gchar *
 ...
 * gs_app_set_sources:
 * @app: a #GsApp
 * @sources: The non-localized short names, e.g. ["lingmo-calculator"]
 *
 ...
 * @app: a #GsApp
 * @source_ids: The source-id, e.g. ["lingmo-calculator;0.134;fedora"]
 ...
 * @app: a #GsApp
 * @source_id: a source ID, e.g. "lingmo-calculator;0.134;fedora"
 *
 ...
 * @app: a #GsApp
 * @project_group: The non-localized project group, e.g. "Lingmo" or "KDE"
 *
...
get_icon_theme (void)
{
	...
	if (display != NULL) {
		...
		gtk_icon_theme_add_resource_path (theme, "/org/lingmo/Software/icons/");
	}
 ...
 * @app: a #GsApp
 * @quality: a #GsAppQuality, e.g. %GS_APP_QUALITY_LOWEST
 * @summary: a string, e.g. "A graphical calculator for Lingmo"
 *
 ...
 * @app: a #GsApp
 * @quality: a #GsAppQuality, e.g. %GS_APP_QUALITY_LOWEST
 * @description: a string, e.g. "Lingmo Calculator is a graphical calculator for Lingmo..."
 *
 ...
 * @app: a #GsApp
 * @kind: a #AsLaunchableKind, e.g. %AS_LAUNCHABLE_KIND_DESKTOP_ID
 * @launchable: a way to launch, e.g. "org.lingmo.Sysprof2.desktop"
 *
 ...
 * "fedoraproject.org" or "sdk.lingmo.org".
 *
 * Returns: a string, or %NULL for unset
 *
 * Since: 3.22
...
calculate_key_colors (GsApp *app)
{
	 ...
	 * <component>
	 *   <custom>
	 *     <value key="LingmoSoftware::key-colors">[(124, 53, 77), (99, 16, 0)]</value>
	 *   </custom>
	 * </component>
	 * ]|
	 */
	overrides_str = gs_app_get_metadata_item (app, "LingmoSoftware::key-colors");
	if (overrides_str != NULL) {
		...
		if (overrides != NULL && g_variant_n_children (overrides) > 0) {
			...
			g_warning ("Invalid value for LingmoSoftware::key-colors for %s: %s",
		...
		if (icon_paintable != NULL) {
			...
			if (path != NULL) {
				...
				for (guint i = 0; names != NULL && names[i] != NULL && pb_small == NULL; i++) {
					...
					resource_path = g_strconcat ("/org/lingmo/Software/icons/scalable/apps/", names[i], ".svg", NULL);
 ...
 * are set by the user (using `LingmoSoftware::key-colors`). %FALSE
...
gs_app_class_init (GsAppClass *klass)
{
	 ...
	 * GsApp:special-kind:
	 *
	 * Lingmo Software specific occupation of the #GsApp entity
...
/**
 * gs_app_new:
 * @id: an application ID, or %NULL, e.g. "org.lingmo.Software.desktop"
 *
 ...
 *	`system/flatpak/lingmo/desktop/org.lingmo.Software.desktop/master`
 *
...
gs_app_get_packaging_format (GsApp *app)
{
	...
	packaging_format = gs_app_get_metadata_item (app, "LingmoSoftware::PackagingFormat");
