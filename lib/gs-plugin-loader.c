 ...
 *
 * This function is intended to be used by internal lingmo-software code.
 *
 * Since: 42
 */
void
                               ...
                               GsPluginRefineFlags  refine_flags)
{
	...
	if (g_strcmp0 (gs_app_get_id (app), "lingmo-system-monitor-kde.desktop") == 0) {
...
add_app_to_install_queue (GsPluginLoader *plugin_loader, GsApp *app)
{
	...
	g_source_set_name (source, "[lingmo-software] emit_pending_apps_idle");
				   ...
				   GsPluginLoader *plugin_loader)
{
	...
	/* plugin now allowing lingmo-software to show updates panel */
	if (allow_updates) {
					...
					GsPluginLoader *plugin_loader)
{
	...
	gs_plugin_loader_app_create_async (plugin_loader, "system/*/*/org.lingmo.Software.desktop/*",
...
finish_setup_get_bus (GTask *task)
{
	...
	if (plugin_loader->locations->len == 0) {
		...
		g_ptr_array_add (locations, g_build_filename (LIBDIR, "lingmo-software", filename, NULL));
	} else {
...
gs_plugin_loader_class_init (GsPluginLoaderClass *klass)
{
	 ...
	 *
	 * Whether updates and upgrades are managed by lingmo-software.
	 *
...
gs_plugin_loader_init (GsPluginLoader *plugin_loader)
{
	...
	plugin_loader->settings = g_settings_new ("org.lingmo.software");
	...
	tmp = g_getenv ("Lingmo_SOFTWARE_COMPATIBLE_PROJECTS");
	if (tmp == NULL) {
 ...
 *
 * This is intended to be used by internal lingmo-software code. Plugin and UI
