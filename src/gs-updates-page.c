...
{
	g_autoptr(GError) error = NULL;
	if (!g_spawn_command_line_async ("lingmo-control-center wifi", &error))
...
gs_updates_page_class_init (GsUpdatesPageClass *klass)
{
	...
	gtk_widget_class_set_template_from_resource (widget_class, "/org/lingmo/Software/gs-updates-page.ui");
...
gs_updates_page_init (GsUpdatesPage *self)
{
	...
	self->desktop_settings = g_settings_new ("org.lingmo.desktop.interface");
