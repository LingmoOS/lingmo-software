...
gs_overview_page_dup_deployment_featured_filename (void)
{
	...
	filename = g_build_filename (SYSCONFDIR, "lingmo-software", FILENAME, NULL);
	...
	for (guint i = 0; sys_dirs != NULL && sys_dirs[i]; i++) {
		g_autofree gchar *tmp = g_build_filename (sys_dirs[i], "lingmo-software", FILENAME, NULL);
...
gs_overview_page_class_init (GsOverviewPageClass *klass)
{
	...
	gtk_widget_class_set_template_from_resource (widget_class, "/org/lingmo/Software/gs-overview-page.ui");
