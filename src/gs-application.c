		 ...
		 gpointer       user_data)
{
	...
	adw_show_about_window (app->main_window,
			       ...
			       "developer-name", _("The Lingmo Project"),
			       "version", get_version(),
			       "website", "https://apps.lingmo.org/Software",
			       "support-url", "https://discourse.lingmo.org/tag/lingmo-software",
			       "issue-url", "https://gitlab.gnome.org/GNOME/lingmo-software/-/issues",
	...
	gtk_window_set_title (GTK_WINDOW (dialog), _("About Lingmo Software"));

	/* TRANSLATORS: well, we seem to think so, anyway */
	gtk_about_dialog_set_comments (dialog, _("This program is developed based on "
									"GNOME Software"));
		       ...
		       gpointer       data)
{
	...
	a = gs_app_new (NULL);
	gs_app_set_metadata (a, "LingmoSoftware::from-url", url);
	gs_shell_reset_state (app->shell);
	gs_shell_show_app (app->shell, a);
}
...
_copy_file_to_cache (GFile *file_src, GError **error)
{
	...
	cache_dir = g_dir_make_tmp ("lingmo-software-XXXXXX", error);
	if (cache_dir == NULL)
		return NULL;
...
gs_application_startup (GApplication *application)
{
	...
	/* allow for debugging */
	tmp = g_getenv ("Lingmo_SOFTWARE_PLUGINS_BLOCKLIST");
	if (tmp != NULL)
		plugin_blocklist = g_strsplit (tmp, ",", -1);
	tmp = g_getenv ("Lingmo_SOFTWARE_PLUGINS_ALLOWLIST");
	if (tmp != NULL)
...
gs_application_constructed (GObject *object)
{
	 ...
	 * org.lingmo.Software, e.g. for the development profile (when
...
gs_application_handle_local_options (GApplication *app, GVariantDict *options)
{
	...
	if (g_variant_dict_contains (options, "prefer-local"))
		g_setenv ("Lingmo_SOFTWARE_PREFER_LOCAL", "true", TRUE);

	if (g_variant_dict_contains (options, "version")) {
		g_print ("lingmo-software %s\n", get_version());
		return 0;
	}
