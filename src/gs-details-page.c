				      ...
				      gboolean sensitive)
{
	...
	packaging_icon = gs_app_get_metadata_item (self->app, "LingmoSoftware::PackagingIcon");
	if (packaging_icon == NULL)
		packaging_icon = "package-x-generic-symbolic";

	packaging_base_css_color = gs_app_get_metadata_item (self->app, "LingmoSoftware::PackagingBaseCssColor");
							...
							GsDetailsPage *self)
{
	...
	if (gs_app_get_license_is_free (self->app)) {
		...
		if (gs_app_get_license (self->app) == NULL) {
			uri = "https://gitlab.gnome.org/GNOME/lingmo-software/-/wikis/software-metadata#license";
		} else {
			...
			if (license_url != NULL && *license_url != '\0') {
				 ...
				 * software. This is a page on the lingmo-software wiki for now,
				...
				uri = "https://gitlab.gnome.org/GNOME/lingmo-software/-/wikis/Software-licensing";
			}
		}
	}
                                   ...
                                   gpointer user_data)
{
	 ...
	 * https://gitlab.gnome.org/GNOME/lingmo-software/-/issues/1192
	 *
...
gs_details_page_refresh_all (GsDetailsPage *self)
{
	...
	tmp = gs_app_get_metadata_item (self->app, "LingmoSoftware::problems");
	if (tmp == NULL || *tmp == '\0') {
		...
		if (runtime != NULL)
			tmp = gs_app_get_metadata_item (runtime, "LingmoSoftware::problems");
	}
	...
	tmp = gs_app_get_metadata_item (self->app, "LingmoSoftware::EolReason");
	if (tmp == NULL || *tmp == '\0') {
		/* Show runtime EOL on the apps which use them, unless they have their own EOL */
		GsApp *runtime = gs_app_get_runtime (self->app);
		if (runtime != NULL)
			tmp = gs_app_get_metadata_item (runtime, "LingmoSoftware::EolReason");
	}
...
gs_details_page_class_init (GsDetailsPageClass *klass)
{
	...
	gtk_widget_class_set_template_from_resource (widget_class, "/org/lingmo/Software/gs-details-page.ui");
...
gs_details_page_init (GsDetailsPage *self)
{
	...
	self->settings = g_settings_new ("org.lingmo.software");
