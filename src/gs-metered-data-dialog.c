				    ...
				    gpointer   user_data)
{
	...
	if (!g_spawn_command_line_async ("lingmo-control-center wifi", &error_local)) {
		g_warning ("Error opening Lingmo Control Center: %s",
			   error_local->message);
		return;
	}
}
...
gs_metered_data_dialog_class_init (GsMeteredDataDialogClass *klass)
{
	...
	gtk_widget_class_set_template_from_resource (widget_class, "/org/lingmo/Software/gs-metered-data-dialog.ui");
