...
/* The days below are discussed at https://gitlab.gnome.org/GNOME/lingmo-software/-/issues/947
   and https://wiki.lingmo.org/Design/Apps/Software/Updates#Tentative_Design */
static gboolean
			  ...
			  gpointer data)
{
	...
	/* do not show if lingmo-software is already open */
	if (gs_application_has_active_window (monitor->application))
		return;
...
gs_update_monitor_init (GsUpdateMonitor *monitor)
{
	GNetworkMonitor *network_monitor;
	g_autoptr(GError) error = NULL;
	monitor->settings = g_settings_new ("org.lingmo.software");
