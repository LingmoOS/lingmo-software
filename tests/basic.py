...
settings = Gio.Settings.new("org.lingmo.desktop.interface")
...
try:
    ...
    super_menu = root.application('lingmo-shell').child(roleName='menu', name='Software')
    super_menu.click()
    root.application('lingmo-shell').child(roleName='label', name='Software Sources').click()
    assert (len(app.children) == 2)
    sources_dialog = app.children[-1]
    assert (sources_dialog.child(roleName='label', name='Software Sources'))
finally:
    os.system("killall lingmo-software")
