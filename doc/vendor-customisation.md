Vendor customisation of Lingmo Software
======================================

Lingmo Software is in an unusual position in a distribution, as it lies at the
interface of the Lingmo project and the distribution’s packaging and release
infrastructure. Lingmo Software is the user interface which a lot of users will
...
For this reason, Lingmo Software has a few ways which vendors can use to
...
please [submit a merge request](https://gitlab.gnome.org/GNOME/lingmo-software/-/merge_requests/new)
to document it.

Likewise, if your distribution would like to customise lingmo-software in a way
which isn’t currently supported, please
[open a new issue](https://gitlab.gnome.org/GNOME/lingmo-software/-/issues/new?issue%5Bmilestone_id%5D=)
...
The principles which guide vendor customisation features in Lingmo Software are:
 * Avoid requiring vendor specific code.
   - Otherwise vendors have to maintain and test Lingmo Software plugins, which
     is a lot of work.
 ...
 * Don’t require downstream patching of Lingmo Software, although configure-time
   arguments are OK.
 ...
 * Be mindful of release cadences.
   - If customisations related to a new OS version were tied to the release
     cycle of Lingmo Software, a new Lingmo Software packaging release would have
...
If your distribution has a specific Lingmo Software plugin providing its upgrade
...
in order:
 * `${DATADIR}/lingmo-software/backgrounds/${os_id}-${version}.png`
 * `${DATADIR}/lingmo-software/backgrounds/${os_id}.png`
 ...
 * Carousel on the overview page: Applications are included if they have
   `component/custom/value[@key='LingmoSoftware::FeatureTile]` or
   `component/custom/value[@key='LingmoSoftware::FeatureTile-css]` set in their
 ...
 * “Editor’s Choice” on the overview page: Applications are included if they
   have `component/kudos/kudo[text()='LingmoSoftware::popular']` set in their
...
<components>
  <component merge="append">
    ...
    <custom>
      <value key="LingmoSoftware::FeatureTile">True</value>
    </custom>

    <!-- Include the app in the “Editor’s Choice” section on the overview page. -->
    <kudos>
      <kudo>LingmoSoftware::popular</kudo>
    </kudos>
    ...
    <categories>
      <!-- Note that, due to a bug (https://gitlab.gnome.org/GNOME/lingmo-software/-/issues/1649),
...
image-based distributions – Lingmo Software can download distro-specific
...
file. Lingmo Software must be configured with `-Dexternal_appstream=true` for
this to work.

Lingmo Software will periodically check and download any updates to these
files, and will cache them locally. Ensure the `If-Modified-Since` HTTP header
functions correctly on your server, or Lingmo Software’s caching will be
ineffective.

The `external-appstream-urls` mechanism may change in future.

Lingmo Software ships a default list of featured applications, chosen to match
the [Lingmo Circle](https://circle.lingmo.org/). See
`data/assets/org.lingmo.Software.Featured.xml` for this list, and for an example
of the metainfo XML needed to feature or highlight applications. See
`data/assets/org.lingmo.Software.Curated.xml` for a default hard-coded list of
curated high quality applications, which is displayed in the “Editor’s Choice”
section of the overview page.

Pass `-Ddefault_featured_apps=false` when configuring Lingmo Software to disable
...
The first file is `org.lingmo.Software.DeploymentFeatured.xml`, which works similarly
to `org.lingmo.Software.Featured.xml` and should be saved beside it in an appstream
directory. It sets the `LingmoSoftware::DeploymentFeatured` key on apps which should
...
It is a semicolon-separated list of `LingmoSoftware::DeploymentFeatured` key values, thus
...
the `lingmo-software/deployment-featured.ini` file. The first directory containing
it will be used. The relevant file names are `/etc/xdg/lingmo-software/deployment-featured.ini`,
`/usr/local/share/lingmo-software/deployment-featured.ini` and
`/usr/share/lingmo-software/deployment-featured.ini`.

Any changes to these files, including adding or removing them, will only be noticed
when lingmo-software is restarted.
